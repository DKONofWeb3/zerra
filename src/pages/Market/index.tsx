...
function CoinCard({ coin, rank }: { coin: TrendingCoin; rank: number }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-card border border-white/[0.06] p-5 flex flex-col gap-4 hover:border-white/[0.12] transition-all hover:-translate-y-0.5"
      style={{ background: "rgb(var(--bg-card))" }}>
      <div aria-hidden className="absolute inset-x-0 top-0 h-px pointer-events-none"
        style={{ background: "linear-gradient(90deg, transparent, rgb(255 255 255 / 0.10), transparent)" }} />
      {coin.data?.sparkline && (
        <img src={coin.data.sparkline} alt="" aria-hidden className="absolute bottom-0 left-0 right-0 w-full opacity-[0.15] pointer-events-none" style={{ height: "50%", objectFit: "fill" }} />
      )}
      <div className="relative flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="relative">
            ...
            <span className="absolute -top-1 -left-1 w-5 h-5 rounded-full bg-bg-elevated border border-white/[0.08] text-[9px] font-bold text-fg-tertiary flex items-center justify-center">{rank}</span>
          </div>
          <div>
        ...
        <span className={cn("px-2 py-0.5 rounded-full text-[11px] font-semibold tabular-nums", isUp ? "text-success" : "text-danger")}
          style={{ backgroundColor: isUp ? "rgb(var(--success) / 0.12)" : "rgb(var(--danger) / 0.12)" }}>
          {isUp ? "+" : ""}{change.toFixed(1)}%
        </span>
      </div>
      <div className="relative">
        <p className="text-[22px] font-display font-medium text-gradient leading-none">{coin.data?.price ?? "—"}</p>
        <p className="text-[11.5px] text-fg-tertiary mt-1.5">Market cap rank #{coin.market_cap_rank ?? "—"}</p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          {topic.hashtags.slice(0, 2).map((tag) => (
            <span key={tag} className="text-[11px] text-fg-tertiary border border-white/[0.06] px-2 py-0.5 rounded-full">{tag}</span>
          ))}
        </div>
      </div>
    </button>
...
function TopicPanel({ topic, onClose }: { topic: Topic; onClose: () => void }) {
  return (
    <>
      ...
      {/* Panel */}
      <div
        className="fixed top-0 right-0 bottom-0 w-[440px] z-40 flex flex-col overflow-hidden border-l border-white/[0.06]"
      ...
      >
        ...
        <div className="flex items-start justify-between gap-4 p-6 border-b border-white/[0.06]">
          ...
          <button onClick={onClose} className="shrink-0 w-8 h-8 rounded-lg border border-white/[0.06] bg-bg-elevated grid place-items-center text-fg-tertiary hover:text-fg-primary transition-colors mt-1">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {topic.hashtags.map((tag) => (
                <span key={tag} className="px-3 py-1.5 rounded-full text-[12.5px] font-medium border border-brand/25 bg-brand/10 text-brand">{tag}</span>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-2">
              {topic.ideas.map((idea, i) => (
                <div key={i} className="flex items-start gap-3 p-3 rounded-xl border border-white/[0.04] bg-bg-elevated/40">
                  <span className="w-5 h-5 rounded-full bg-brand/15 border border-brand/25 text-[10px] font-bold text-brand flex items-center justify-center shrink-0 mt-0.5">{i + 1}</span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {topic.relatedCoins.map((coin) => (
                <span key={coin} className="px-3 py-1.5 rounded-full text-[12.5px] border border-white/[0.08] bg-bg-elevated text-fg-secondary">{coin}</span>
              ))}
            </div>
          </div>
    ...
    <div className="pb-12 space-y-10">
      <div className="pt-2">
        <div className="flex items-center gap-2.5 text-fg-tertiary">
          ...
          <span className="ml-auto text-[11.5px] px-2.5 py-1 rounded-full border border-white/[0.06] bg-bg-elevated text-fg-muted">Updates every 5 min</span>
        </div>
        <h2 className={cn("mt-4 font-display font-medium tracking-[-0.03em]", "text-[64px] leading-[0.95]", "bg-clip-text text-transparent", "bg-gradient-to-b from-white via-white to-[#7d8aa8]")}>
          Market
        </h2>
      </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="px-2.5 py-0.5 rounded-full text-[11px] font-medium border border-white/[0.06] bg-bg-elevated text-fg-tertiary">Top 8 on CoinGecko</span>
        </div>
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
            {[1,2,3,4,5,6,7,8].map((i) => <div key={i} className="h-[180px] rounded-card border border-white/[0.06] animate-pulse" style={{ background: "rgb(var(--bg-card))" }} />)}
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
            {coins.map((coin, i) => <CoinCard key={coin.id} coin={coin} rank={i + 1} />)}
          </div>
        )}
      </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          ...
          <span className="text-[12px] text-fg-tertiary">Click any topic for content ideas</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-4">
          {TRENDING_TOPICS.map((topic, i) => (
            <TopicCard key={i} topic={topic} onClick={() => setSelected(topic)} />
          ))}
        </div>
      </div>
